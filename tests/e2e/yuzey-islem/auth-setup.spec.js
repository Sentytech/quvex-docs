/**
 * Yuzey Islem Kaplama E2E — Auth Setup
...
const authFile    = path.join(authDir, 'yuzey-user.json')
const sessionFile = path.join(authDir, 'yuzey-session.json')
const tenantFile  = path.join(authDir, 'yuzey-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Guven Kaplama San Test ${TS}`,
  subdomain:   `yuzeyislem${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `yuzeyislem${TS}@yuzeytest.com`,
  phone:       '5321234567',
...
setup('quvex.io — yuzey islem tenant olustur ve login ol', async ({ page, request }) => {
  ...
  const standardUnits = ['ADET', 'KG', 'METRE', 'LİTRE', 'M²', 'M³', 'PAKET', 'KUTU', 'SET', 'TON', 'CM', 'MM', 'GR', 'PANEL']
