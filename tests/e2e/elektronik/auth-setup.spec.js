/**
 * Elektronik Kart Montaj E2E — Auth Setup
...
const authFile    = path.join(authDir, 'elektronik-user.json')
const sessionFile = path.join(authDir, 'elektronik-session.json')
const tenantFile  = path.join(authDir, 'elektronik-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Delta Elektronik Savunma Test ${TS}`,
  subdomain:   `elektronik${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `elektronik${TS}@elektroniktest.com`,
  phone:       '5321234567',
...
setup('quvex.io — elektronik tenant olustur ve login ol', async ({ page, request }) => {
