/**
 * Kalip ve Takim Imalati E2E — Auth Setup
...
const authFile    = path.join(authDir, 'kalip-user.json')
const sessionFile = path.join(authDir, 'kalip-session.json')
...
const TEST_TENANT = {
  companyName: `Celik Kalip Savunma Test ${TS}`,
  subdomain:   `kaliptest${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `kalip${TS}@kaliptest.com`,
  phone:       '5321234567',
...
setup('quvex.io — Kalip tenant olustur ve login ol', async ({ page, request }) => {
  ...
  if (process.env.QUVEX_TEST_EMAIL && process.env.QUVEX_TEST_PASSWORD) {
    ...
    console.log(`[auth-setup] Yeni Kalip tenant: ${TEST_TENANT.email}`)
