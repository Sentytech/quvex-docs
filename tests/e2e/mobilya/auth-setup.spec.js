/**
 * Mobilya Imalati E2E — Auth Setup
...
const authFile    = path.join(authDir, 'mobilya-user.json')
const sessionFile = path.join(authDir, 'mobilya-session.json')
const tenantFile  = path.join(authDir, 'mobilya-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Oz Ahsap Mobilya Test ${TS}`,
  subdomain:   `mobilya${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `mobilya${TS}@mobilyatest.com`,
  phone:       '5321234567',
...
setup('quvex.io — mobilya tenant olustur ve login ol', async ({ page, request }) => {
  ...
  const standardUnits = ['ADET', 'KG', 'METRE', 'LİTRE', 'M²', 'M³', 'PAKET', 'KUTU', 'SET', 'TON', 'CM', 'MM', 'GR', 'RULO', 'KIT']
