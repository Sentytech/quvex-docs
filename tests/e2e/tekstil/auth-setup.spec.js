/**
 * Tekstil Üretimi E2E — Auth Setup
...
const authFile    = path.join(authDir, 'tekstil-user.json')
const sessionFile = path.join(authDir, 'tekstil-session.json')
const tenantFile  = path.join(authDir, 'tekstil-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Aydin Tekstil Uretim Test ${TS}`,
  subdomain:   `tekstil${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `tekstil${TS}@tekstiltest.com`,
  phone:       '5321234567',
...
setup('quvex.io — tekstil tenant olustur ve login ol', async ({ page, request }) => {
