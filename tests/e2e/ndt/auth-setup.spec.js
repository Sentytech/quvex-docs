/**
 * NDT Muayene Laboratuvari E2E — Auth Setup
...
const authFile    = path.join(authDir, 'ndt-user.json')
const sessionFile = path.join(authDir, 'ndt-session.json')
...
const TEST_TENANT = {
  companyName: `Guventest NDT Test ${TS}`,
  subdomain:   `ndttest${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `ndt${TS}@ndttest.com`,
  phone:       '5321234567',
...
setup('quvex.io — NDT tenant olustur ve login ol', async ({ page, request }) => {
  ...
  if (process.env.QUVEX_TEST_EMAIL && process.env.QUVEX_TEST_PASSWORD) {
    ...
    console.log(`[auth-setup] Yeni NDT tenant: ${TEST_TENANT.email}`)
