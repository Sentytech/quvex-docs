/**
 * Kaynak Atolyesi E2E — Auth Setup
...
const authFile    = path.join(authDir, 'kaynak-user.json')
const sessionFile = path.join(authDir, 'kaynak-session.json')
const tenantFile  = path.join(authDir, 'kaynak-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Ozdemir Kaynak Muhendislik Test ${TS}`,
  subdomain:   `kaynak${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `kaynak${TS}@kaynaktest.com`,
  phone:       '5321234567',
...
setup('quvex.io — kaynak tenant olustur ve login ol', async ({ page, request }) => {
