...
const authFile    = path.join(authDir, 'dokum-user.json')
const sessionFile = path.join(authDir, 'dokum-session.json')
const tenantFile  = path.join(authDir, 'dokum-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Anatolya Dokum Havacilik Test ${TS}`,
  subdomain:   `dokumtest${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `dokum${TS}@dokumtest.com`,
  phone:       '5321234567',
...
setup('quvex.io — Dokum tenant olustur ve login ol', async ({ page, request }) => {
  ...
  if (process.env.QUVEX_TEST_EMAIL && process.env.QUVEX_TEST_PASSWORD) {
    ...
    console.log(`[auth-setup] Yeni Dokum tenant: ${TEST_TENANT.email}`)
