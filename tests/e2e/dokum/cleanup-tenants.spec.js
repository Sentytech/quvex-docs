/**
 * Hassas Döküm E2E — Cleanup
 * Test sonrasi olusturulan Döküm tenant'i temizler.
 */
...
test('Dokum — test tenant temizle', async ({ request }) => {
  if (!fs.existsSync(tenantFile)) {
    console.log('[cleanup] dokum-tenant.json bulunamadi, atlanıyor.')
    return
  }
  ...
  console.log(`[cleanup] Döküm tenant silme: ${delRes.status()}`)

  // Auth dosyalarini temizle
  const files = ['dokum-user.json', 'dokum-session.json', 'dokum-tenant.json']
