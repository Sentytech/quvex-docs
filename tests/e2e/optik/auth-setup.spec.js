/**
 * Optik Hassas Montaj E2E — Auth Setup
...
const authFile    = path.join(authDir, 'optik-user.json')
const sessionFile = path.join(authDir, 'optik-session.json')
...
const TEST_TENANT = {
  companyName: `Vizyon Optik Savunma Test ${TS}`,
  subdomain:   `optiktest${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `optik${TS}@optiktest.com`,
  phone:       '5321234567',
...
setup('quvex.io — Optik tenant olustur ve login ol', async ({ page, request }) => {
  ...
  if (process.env.QUVEX_TEST_EMAIL && process.env.QUVEX_TEST_PASSWORD) {
    ...
    console.log(`[auth-setup] Yeni Optik tenant: ${TEST_TENANT.email}`)
