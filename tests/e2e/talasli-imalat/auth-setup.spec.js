...
const authFile    = path.join(authDir, 'talasli-user.json')
const sessionFile = path.join(authDir, 'talasli-session.json')
const tenantFile  = path.join(authDir, 'talasli-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Talasli Imalat Test ${TS}`,
  subdomain:   `talasli${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `talasli${TS}@talaslitest.com`,
  phone:       '5321234567',
...
setup('quvex.io — talasli imalat tenant olustur ve login ol', async ({ page, request }) => {
  ...
  // ── Adım 6: Birimler seed — yeni tenant Units tablosu boş gelir ──
