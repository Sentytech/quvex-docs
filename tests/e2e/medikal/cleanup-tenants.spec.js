/**
 * Medikal Cihaz — Test tenant temizleme
 * Calistirma: npx playwright test --config=playwright.config.js cleanup-tenants.spec.js
 */
...
// Her test kosumundan sonra buraya ID ekle (medikal-tenant.json'dan bakarak)
const TEST_TENANT_IDS = [
  // Ornek: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', // medikalXXXXXX tenant
]

base('Medikal cihaz test tenantlarini sil', async ({ page }) => {
  ...
  for (const tenantId of TEST_TENANT_IDS) {
    ...
    if ([200, 204, 404].includes(res.status)) {
      console.log(`[cleanup] OK ${tenantId} (${res.status})`)
      deleted++
    } else {
      console.warn(`[cleanup] FAIL ${tenantId} — status ${res.status}`)
    }
  }
