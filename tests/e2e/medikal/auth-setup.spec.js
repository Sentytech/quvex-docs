...
const authFile    = path.join(authDir, 'medikal-user.json')
const sessionFile = path.join(authDir, 'medikal-session.json')
const tenantFile  = path.join(authDir, 'medikal-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `MedTek Cerrahi Test ${TS}`,
  subdomain:   `medikal${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `medikal${TS}@medikaltest.com`,
  phone:       '5321234567',
...
setup('quvex.io — medikal cihaz tenant olustur ve login ol', async ({ page, request }) => {
  if (!fs.existsSync(authDir)) fs.mkdirSync(authDir, { recursive: true })

  // -- Adim 1: Env var verilmisse direkt login --
  if (process.env.QUVEX_TEST_EMAIL && process.env.QUVEX_TEST_PASSWORD) {
    ...
    if (registerRes.ok()) {
      ...
      console.warn('[auth-setup] Fallback: cnc sector ile yeniden deneniyor')
      // medikal sektoru yoksa cnc fallback
      TEST_TENANT.sector = 'cnc'
      const retryRes = await request.post(`${API}/register`, {
        headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
        data: TEST_TENANT,
      }).catch(() => null)
      if (!retryRes?.ok()) {
        ...
        const body2 = await retryRes.json().catch(() => ({}))
        fs.writeFileSync(tenantFile, JSON.stringify({
          email: TEST_TENANT.email, password: TEST_TENANT.password,
          tenantId: body2.tenantId, createdAt: new Date().toISOString(),
        }, null, 2))
      }
    }
