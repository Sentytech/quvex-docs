/**
 * Otomotiv Yan Sanayi E2E — Auth Setup
...
const authFile    = path.join(authDir, 'otomotiv-user.json')
const sessionFile = path.join(authDir, 'otomotiv-session.json')
const tenantFile  = path.join(authDir, 'otomotiv-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Aslan Otomotiv Parca Test ${TS}`,
  subdomain:   `otomotiv${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `otomotiv${TS}@otomotivtest.com`,
  phone:       '5321234567',
...
setup('quvex.io — otomotiv tenant olustur ve login ol', async ({ page, request }) => {
