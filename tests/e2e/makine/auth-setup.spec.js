...
const authFile    = path.join(authDir, 'makine-user.json')
const sessionFile = path.join(authDir, 'makine-session.json')
const tenantFile  = path.join(authDir, 'makine-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Teknik Makine Muhendislik Test ${TS}`,
  subdomain:   `makine${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `makine${TS}@makinetest.com`,
  phone:       '5321234567',
...
setup('quvex.io — makine tenant olustur ve login ol', async ({ page, request }) => {
