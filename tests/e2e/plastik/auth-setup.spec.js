/**
 * Plastik Enjeksiyon E2E — Auth Setup
...
const authFile    = path.join(authDir, 'plastik-user.json')
const sessionFile = path.join(authDir, 'plastik-session.json')
const tenantFile  = path.join(authDir, 'plastik-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Ozkan Plastik San Test ${TS}`,
  subdomain:   `plastik${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `plastik${TS}@plastiktest.com`,
  phone:       '5321234567',
...
setup('quvex.io — plastik tenant olustur ve login ol', async ({ page, request }) => {
