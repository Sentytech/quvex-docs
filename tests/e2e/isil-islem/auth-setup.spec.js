/**
 * Isil Islem Fason E2E — Auth Setup
...
const authFile    = path.join(authDir, 'isil-user.json')
const sessionFile = path.join(authDir, 'isil-session.json')
const tenantFile  = path.join(authDir, 'isil-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Atas Metalurji San Test ${TS}`,
  subdomain:   `isilislem${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `isilislem${TS}@isiltest.com`,
  phone:       '5321234567',
...
setup('quvex.io — isil islem tenant olustur ve login ol', async ({ page, request }) => {
  ...
  if (process.env.QUVEX_TEST_EMAIL && process.env.QUVEX_TEST_PASSWORD) {
    ...
    if (registerRes.ok()) {
      ...
      console.warn('[auth-setup] Fallback: super admin ile devam (sector=cnc)')
      // isil sektoru yoksa cnc fallback
