...
const TEST_TENANT_IDS = [
  // Örnek: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', // savunmaozelXXXXXX tenant
]

base('Savunma ozel proses test tenantlarini sil', async ({ page }) => {
  if (TEST_TENANT_IDS.length === 0) { console.log('[cleanup] Silinecek tenant yok.'); return }
  ...
  for (const tenantId of TEST_TENANT_IDS) {
    ...
    if ([200, 204, 404].includes(res.status)) { console.log(`[cleanup] ✅ ${tenantId}`); deleted++ }
    else console.warn(`[cleanup] ❌ ${tenantId} — ${res.status}`)
  }
