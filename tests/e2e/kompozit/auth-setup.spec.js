/**
 * Kompozit İmalat E2E — Auth Setup
...
const authFile    = path.join(authDir, 'kompozit-user.json')
const sessionFile = path.join(authDir, 'kompozit-session.json')
const tenantFile  = path.join(authDir, 'kompozit-tenant.json')

const API = process.env.API_URL || 'https://api.quvex.io'
const TS  = Date.now().toString().slice(-6)

const TEST_TENANT = {
  companyName: `Kartal Kompozit Havacilik Test ${TS}`,
  subdomain:   `kompozit${TS}`,
  fullName:    'Test Admin',
  email:       process.env.QUVEX_TEST_EMAIL || `kompozit${TS}@kompozittest.com`,
  phone:       '5321234567',
...
setup('quvex.io — kompozit tenant olustur ve login ol', async ({ page, request }) => {
