...
## 8. Sprint 11 Guvenlik Sertlestirme (2026-04-12)

### Yetkilendirme Duzeltmeleri
- [x] **Multi-role permission merge:** Kullanici birden fazla role sahipse TUM rollerin izinleri yuklenir (onceden sadece ilk rol okunuyordu — ayricalik kaybi riski kapatildi)
- [x] `/Account/my-permissions` endpoint'i tum authenticated kullanicilara acildi (daha once sadece admin roluyle erisilebiliyordu)
- [x] `bootstrap-permissions` endpoint'ine tenant filtresi eklendi (cross-tenant izin sizintisi onlendi)
- [x] `userRoleAssign` endpoint'i yalnizca super admin'e acik hale getirildi
- [x] `/metrics` endpoint'i artik authentication gerektiriyor (onceden public idi)

### Tenant IDOR (Insecure Direct Object Reference) Fix
- [x] `Account/GetAllAsync` — tenant filtresi eklendi
- [x] `Account/Get(id)` — ayni tenant kontrolu
- [x] `Account/DeleteUser` — cross-tenant silme engellendi
- [x] `Account/PutUser` — cross-tenant guncelleme engellendi
- [x] `Account/GetMyPermissions` — tenant scope ile sinirlandirildi
- [x] `X-Tenant-Id` header spoofing korumasi: istemciden gelen header yok sayilir, tenant context token'dan cozulur

### Veri Sifreleme (Hassas Alanlar)
- [x] **AES-256-CBC encryption:** IBAN, banka hesap numarasi, vergi numarasi alanlari at-rest sifrelenmis olarak saklanir
- [x] Sifreleme anahtari environment variable uzerinden yonetilir (`QUVEX_FIELD_ENCRYPTION_KEY`)

### PII Koruma ve Loglama
- [x] `RequestLoggingMiddleware` PII redaction: TC kimlik, IBAN, kart no, sifre alanlari loglanmadan once maskelenir
- [x] Sentry PII stripping (API + UI) — hata mesajlarinda kullanici verileri temizlenir

### Dirence (Resilience)
- [x] **Polly circuit breaker:** TCMB kur servisi, PayTR odeme, eInvoice (GIB) entegrasyonlari icin
- [x] 5 ardisik hata sonrasi 30 sn acik devre; half-open probe ile kurtarma

### Tenant Izolasyon Matrisi (Sprint 11 sonu)
| Katman | Kontrol | Durum |
|--------|---------|-------|
| DB | HasQueryFilter (143 entity) | OK |
| API | TenantContext (JWT claim) | OK |
| API | X-Tenant-Id spoof koruma | OK (Sprint 11) |
| API | Account/* IDOR fix | OK (Sprint 11) |
| API | bootstrap-permissions filter | OK (Sprint 11) |
| SignalR | JoinTenantGroup + group isolation | OK |
| SignalR | production-board group | OK (Sprint 11) |
| UI | Response tenant validation interceptor | OK |
| UI | Logout temizligi | OK |

### Sprint 11 Guvenlik Ozeti
- 10+ yetkilendirme/IDOR duzeltmesi
- 3 hassas alan AES-256-CBC sifrelemesi
- PII redaction + Sentry PII stripping
- 3 dis servis icin Polly circuit breaker
- Multi-role permission merge (kritik)
